...
export async function getBackups(_input, _response: Http.OutgoingMessage) {
  ...
  if (!email || !token || !uuid || !name || !world ) {
    ...
    let rs: RealmsServer = c.worlds.getWorld(world);
    _response.write(JSON.stringify(rs.backups()));
  }
}